...
export default function ParentSyllabusDetailScreen() {
    ...
    const fetchSyllabus = useCallback(async () => {
        ...
        try {
            ...
            console.error("Parent syllabus fetch error:", err);
            setSyllabus(null);
        } finally {
            setLoading(false);
            setRefreshing(false);
                ...
                >
                    ...
                    {selectedChild && (
                        <Text className="font-lexend text-sm text-white/70 mb-1">
                            {selectedChild.first_name}'s syllabus{syllabus?.teacher_name ? ` \u2022 ${syllabus.teacher_name}` : ""}
                        </Text>
                    )}
            ...
            {(topic.subtopics || []).length > 0 && (
                ...
                >
                    {topic.subtopics.map((sub, si) => (
                        <SubtopicCard key={sub.id} subtopic={sub} prefix={`${lessonIndex + 1}.${topicIndex + 1}.${si + 1}`} />
                    ))}
                </View>
            )}
        </View>
