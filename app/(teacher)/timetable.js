...
import { timetableService } from "../../services/teacher";
...
export default function TimetableScreen() {
    ...
    const fetchTimetable = useCallback(async () => {
        ...
        try {
            ...
            const res = await timetableService.getDailyTimetable(skid, teacherId, dateStr);
            const data = res?.data || [];
    ...
    // Determine first "next up" for today
    const firstNextUpIndex = useMemo(() => {
        if (!isTodaySelected) return -1;
        for (let i = 0; i < periods.length; i++) {
            const status = getPeriodStatus(periods[i].start_time, periods[i].end_time);
    ...
    return (
        <SafeAreaView className="flex-1 bg-dark-50">
            ...
            >
                {/* Header + Day Selector with white background */}
                ...
                <View className="flex-row px-6 pt-3 gap-2">
                    {weekDays.map((day) => {
                        ...
                        return (
                            ...
                            >
                                <Text
                                    className={`font-lexend-semibold text-xs mb-1 ${
                                        isSelected
                                            ? "text-white/80"
                                            : "text-dark-400"
                                    }`}
                                ...
                                <Text
                                    className={`font-lexend-bold text-xl ${
                                        isSelected
                                            ? "text-white"
                                            : "text-dark-700"
                                    }`}
                        ...
                        <View className="ml-3 pl-8 border-l-2 border-primary-200">
                            {periods.map((period, index) => {
                                ...
                                if (isPastDate) {
                                    ...
                                    if (status === "upcoming" && index === firstNextUpIndex) {
                                        status = "next";
                                    }
                                } else {
                                    status = "upcoming";
...
function TimelineDot({ status }) {
    ...
    return (
        ...
        >
            ...
            >
                {isCompleted && (
                    <Text className="text-white text-[10px] font-bold text-center leading-4">
                        ✓
                    </Text>
                )}
            </View>
...
function OngoingCard({ period }) {
    ...
    const navigateToSyllabus = () => {
        router.push({
            pathname: "/(teacher)/syllabus-detail",
            params: {
        ...
        >
            ...
            <View className="bg-primary-600 px-5 pt-4 pb-5">
                ...
                <View className="flex-row items-center bg-white/20 self-start rounded-full px-3 py-1 mb-3">
                    ...
                    <Text className="font-lexend-bold text-xs text-white">
                        Now
                    </Text>
                </View>
            ...
            <View className="p-5">
                <View className="flex-row gap-3">
                    <View className="flex-1 bg-primary-50 rounded-2xl p-3">
                        <Text className="font-lexend-bold text-[10px] text-primary-400 uppercase tracking-widest mb-1.5">
                            Time
                        </Text>
...
function PeriodCard({ period, status, isCompleted, isNext }) {
    ...
    return (
        ...
        >
            ...
            <View className="mb-2">
                {statusBg ? (
                    <View className={`self-start rounded-full px-2.5 py-0.5 ${statusBg}`}>
                        <Text
                            className={`font-lexend-bold text-[10px] uppercase tracking-widest ${statusColor}`}
                        >
                            {statusLabel}
                        </Text>
                    </View>
            ...
            {/* Subject */}
            <Text className="font-lexend-bold text-[17px] text-dark-800 mt-1">
                {period.subject_name}
            </Text>
            ...
            {period.class_name ? (
                <View className="flex-row items-center mt-1">
                    <Text className="text-dark-400 text-sm mr-1.5">&#x1F4DA;</Text>
                    <Text className="font-lexend text-sm text-dark-500">
                        {period.class_name}
                        {period.section_name ? `-${period.section_name}` : ""}
                    </Text>
                </View>
